...
if (!Avatar)
  throw new Error('Avatar is undefined in InstructorHighlightsSection');
          ...
          {instructors.map((instructor, idx) => (
            ...
            >
              <Card className="flex flex-col items-center p-8 bg-white dark:bg-[color:var(--ai-card-bg)] shadow-lg rounded-2xl border border-[color:var(--ai-card-border)] hover:shadow-xl transition-all duration-300">
                ...
                />
                <h3 className="text-xl font-semibold text-[color:var(--ai-foreground)] mb-1 text-center">
                  {instructor.name}
                </h3>
                <p className="text-sm text-[color:var(--ai-primary)] mb-2 text-center">
                  {instructor.title}
                </p>
                <div className="flex flex-wrap gap-2 mb-3 justify-center">
                  {instructor.specialties.map(spec => (
                    <Chip key={spec} color="primary" variant="flat" size="sm">
                      {spec}
                    </Chip>
                  ))}
                </div>
                <p className="text-[color:var(--ai-muted)] text-center text-sm">
                  {instructor.bio}
                </p>
              </Card>
            </motion.div>
