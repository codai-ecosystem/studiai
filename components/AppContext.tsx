...
export const AppContextProvider = ({
    ...
    async (preferences: Partial<UserPreferences>): Promise<boolean> => {
      ...
      try {
        const preferencesRef = doc(
          firestoreDB,
          `users/${user.uid}/profile/preferences`
        );
  ...
  const getCourseLessons = useCallback(
    async (courseId: string, options?: CacheOptions) => {
      ...
      const cacheKey =
        cacheOptions.cacheKey || generateCacheKey('lessons', courseId);
      ...
      try {
        ...
        const unsubscribe = onSnapshot(
          ...
          querySnapshot => {
            ...
            console.log(
              `Loaded ${querySnapshot.size} lessons for course ${courseId}`
            );
  ...
  const getCourseReviews = useCallback(
    async (courseId: string, options?: CacheOptions) => {
      const cacheOptions = { ...DEFAULT_CACHE_OPTIONS, ...options };
      const cacheKey =
        cacheOptions.cacheKey || generateCacheKey('reviews', courseId);
    ...
    async (
      ...
      try {
        const progressRef = doc(
          firestoreDB,
          `users/${user.uid}/progress/${courseId}_${lessonId}`
        );
    ...
    async (userId: string, courseId: string) => {
      ...
      try {
        ...
        const product = state.products.find(
          (product: any) =>
            product.prices &&
            product.prices.some((price: any) => price.id === priceId)
        );
    ...
    async (options?: CacheOptions) => {
      ...
      if (state.adminAnalytics) {
        ...
        if (state.adminAnalyticsLoadingState !== 'success') {
          dispatch({
            type: 'SET_ADMIN_ANALYTICS_LOADING_STATE',
            payload: 'success',
          });
        }
        return state.adminAnalytics;
        ...
        salesSnapshot.forEach(doc => {
          ...
          if (sale.amount !== undefined) {
            ...
            monthlyRevenue[monthYear] =
              (monthlyRevenue[monthYear] || 0) + sale.amount / 100;
    ...
    async (options?: CacheOptions) => {
      ...
      if (state.adminSettings) {
        ...
        if (state.adminSettingsLoadingState !== 'success') {
          dispatch({
            type: 'SET_ADMIN_SETTINGS_LOADING_STATE',
            payload: 'success',
          });
        }
        return state.adminSettings;
  ...
  const getBookmarkedLessons = useCallback(
    async (options?: CacheOptions) => {
      ...
      if (
        ...
        Object.keys(state.bookmarkedLessons).length > 0
      ) {
        // Check if we need to update loading state
  ...
  const toggleBookmarkLesson = useCallback(
    async (courseId: string, lessonId: string) => {
      if (!user) return;
      const isBookmarked =
        state.bookmarkedLessons?.[courseId]?.includes(lessonId);
    ...
    async (courseId: string) => {
      ...
      try {
        const wishlistDocRef = doc(
          firestoreDB,
          `users/${user.uid}/wishlist/${courseId}`
        );
  ...
  const removeFromWishlist = useCallback(
    async (courseId: string) => {
      ...
      try {
        ...
        const cachedData = loadFromLocalStorage(cacheKey);
        if (cachedData) {
          const updatedWishlist = state.wishlistCourses.filter(
  ...
  useEffect(() => {
    ...
    getProducts(payments, {
      ...
      const filteredProducts = appName
        ? products.filter(
            product => product.metadata && product.metadata.app === appName
          )
        : products;
      ...
      try {
        ...
        if (mounted) {
          querySnapshot.forEach(doc => {
            ...
            dispatch({
              type: 'SET_COURSES',
              payload: { courseId: doc.id, course: data },
            });
            dispatch({
              type: 'SET_COURSE_LOADING_STATE',
    ...
    async (courseId: string, options: CacheOptions = DEFAULT_CACHE_OPTIONS) => {
      ...
      if (options.persist) {
        ...
        if (cachedData && !isCacheExpired(cachedData.metadata)) {
          dispatch({
            type: 'SET_COURSES',
            payload: { courseId, course: cachedData.data },
          });
          dispatch({
            type: 'SET_COURSE_LOADING_STATE',
            payload: { courseId, status: 'success' },
      ...
      dispatch({
        ...
        payload: { key: cacheKey, isPending: true },
      });
      ...
      try {
        ...
        dispatch({
          type: 'SET_PENDING_REQUEST',
          payload: { key: cacheKey, isPending: false },
        });
      }
    },
  ...
  const fetchLessonsForCourse = useCallback(
    async (courseId: string, options: CacheOptions = DEFAULT_CACHE_OPTIONS) => {
      const cacheKey =
        options.cacheKey || generateCacheKey('lessons', courseId);
      ...
      if (options.persist) {
        ...
        if (cachedData && !isCacheExpired(cachedData.metadata)) {
          dispatch({
            type: 'SET_LESSONS',
            payload: { courseId, lessons: cachedData.data },
          });
          dispatch({
            type: 'SET_LESSON_LOADING_STATE',
            payload: { courseId, status: 'success' },
      ...
      try {
        ...
        dispatch({
          type: 'SET_LESSONS',
          payload: { courseId, lessons: lessonsData },
        });
        dispatch({
          type: 'SET_LESSON_LOADING_STATE',
          payload: { courseId, status: 'success' },
  ...
  const getLessonById = useCallback(
    async (courseId: string, lessonId: string): Promise<any> => {
      ...
      try {
        ...
        if (lessonSnap.exists()) {
          ...
          console.log(
            `No lesson found with ID: ${lessonId} in course: ${courseId}`
          );
        ...
        console.error(
          `Error getting lesson ${lessonId} in course ${courseId}:`,
          error
        );
        return null;
      }
