...
const Card = forwardRef<HTMLDivElement, CardProps>((props, ref) => {
  ...
  const defaultClassName =
    'bg-[color:var(--ai-card-bg)]/70 backdrop-blur-sm border border-[color:var(--ai-card-border)]/60 transition-all duration-200'; // Generate shadow classes
  const shadowClasses =
    shadow === 'none'
      ? ''
      : shadow === 'sm'
  ...
  const hoverableClass = isHoverable
    ...
    : '';
  const pressableClass = isPressable
