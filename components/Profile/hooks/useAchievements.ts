...
export default function useAchievements(): AchievementsData {
  ...
  const syncAchievements = async () => {
    ...
    try {
      ...
      const hasReviews = Object.values(reviews).some(courseReviews =>
        Object.values(courseReviews).some(review => review.userId === user.uid)
      );

      // Get user's login streak
      const userProfileRef = doc(
        firestoreDB,
        `users/${user.uid}/profile/stats`
      );
      ...
      for (const definition of achievementDefinitions) {
        ...
        const achievementRef = doc(
          firestoreDB,
          `users/${user.uid}/achievements/${achievementId}`
        );
        ...
        switch (definition.criteria?.type) {
          ...
          case 'lessons_complete':
            shouldUnlock =
              completedLessonsCount >= (definition.criteria.count || 0);
            break;
          case 'multiple_courses':
            shouldUnlock =
              completedCoursesCount >= (definition.criteria.count || 0);
            break;
          case 'first_review':
