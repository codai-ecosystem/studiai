...
export default function useProfileStats() {
  ...
  useEffect(() => {
    if (!user || !userPaidProducts || !courses || userPaidProducts.length === 0)
      return;
    ...
    userPaidProducts.forEach(product => {
      ...
      if (course) {
        ...
        const courseLessonsCount = Object.keys(
          lessonProgress[courseId] || {}
        ).length;
        totalLessons += courseLessonsCount;

        // Calculate completed lessons
        const courseCompletedLessons = Object.values(
          lessonProgress[courseId] || {}
        ...
        if (
          ...
          courseCompletedLessons === courseLessonsCount
        ) {
          completedCourses++;
        }
        ...
        if (lessonProgress[courseId]) {
          Object.entries(lessonProgress[courseId]).forEach(
            ([lessonId, progress]) => {
