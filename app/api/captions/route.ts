  ...
  try {
    ...
    if (isBuild) {
      return NextResponse.json({
        ...
        captionsData: {
          'en-US': {
            url: 'https://example.com/mock-caption-url',
            content: 'Mock caption content',
          },
        },
    ...
    try {
      ...
      for (const language of languages) {
        ...
        const { transcription, captions } = await generateCaptionsFromAudio(
          audioPath,
          language
        );
        ...
        const captionUrl = await uploadCaptionsToStorage(
          captions,
          courseId,
          lessonId,
          language
