      ...
      try {
        ...
        if (!lessonsSnapshot.empty) {
          ...
          lessonsSnapshot.forEach(doc => {
            const data = doc.data();
            console.log(
              `- ${doc.id}: ${data.name || 'Unnamed'} (Status: ${data.status || 'Not set'})`
            );
          });
        } else {
...
export async function getCourseLessons(
  ...
  try {
    ...
    const lessons: Record<string, DocumentData> = {};
    lessonsSnapshot.forEach(doc => {
    ...
    console.log(
      `Fetched ${Object.keys(lessons).length} lessons for course ${courseId}`
    );
    return lessons;
  } catch (error) {
