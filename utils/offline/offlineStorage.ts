    ...
    if (lesson.resources && lesson.resources.length > 0) {
      ...
      for (const resource of lesson.resources) {
        try {
          ...
          console.error(
            `Failed to download resource ${resource.url} for offline use:`,
            error
          );
        }
      }
  ...
  try {
    ...
    if (user?.uid) {
      ...
      if (userDoc.exists()) {
        ...
        const existingIndex = offlineContentList.findIndex(
          (item: OfflineContent) =>
 ...
 * @param type The type of content (lesson or course)
 */
export async function deleteOfflineContent(
  id: string,
  type: 'lesson' | 'course'
): Promise<void> {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const request = store
        .index('idType')
        .openCursor(IDBKeyRange.only([id, type]));
    ...
    if (user?.uid) {
      ...
      if (userDoc.exists()) {
        ...
        const updatedList = offlineContentList.filter(
          (item: OfflineContent) => !(item.id === id && item.type === type)
        );
